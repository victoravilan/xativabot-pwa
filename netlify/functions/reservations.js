...
exports.handler = async (event) => {
  try {
    ...
    const whenLocalHint = data.dateTime || '';
    ...
    const gameFound = data.gameFound || '';
    const gameTotal = data.gameTotal || '';
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.5;color:#222">
        ...
        ${gameCode ? `
          <hr style="border:none;border-top:1px solid #eee;margin:12px 0" />
          <p><b>Juego del Sabor:</b> Código <b>${escapeHtml(gameCode)}</b> (${escapeHtml(String(gameFound))}/${escapeHtml(String(gameTotal))})</p>
        ` : ''}
        <hr style="border:none;border-top:1px solid #eee;margin:12px 0" />
